...
st.set_page_config(page_title="Domestic Tourism Analytics", page_icon="📈",layout="wide")
st.header("Domestic Tourism Analytics📈")
@st.cache_data(persist=True)
def getData():
    res=pd.read_csv("domestic_visitors.csv")
    return res
domestic=getData()
@st.cache_data(persist=True)
def getCAGRData():
    res=pd.read_csv("pages\cagr.csv")
    return res
cfs=getCAGRData()
#st.dataframe(domestic)
Year=st.multiselect(
    "Select Year",
    options=domestic["year"].unique(),
    default=domestic["year"].unique(),
)

dfs=domestic.query("year==@Year")
#st.dataframe(dfs)
visitors=int(dfs["visitors"].sum())
district=int(domestic["district"].nunique())
cagr=cfs["CAGR"].mean()
